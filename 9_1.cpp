...
namespace ropeBridge {
    ...
    int execute(const std::vector<std::pair<char, int>> &instructions, int n) {
        ...
        pos &head = knots.front();
        pos &tail = knots.back();

        for (const std::pair<char, int> &instruction: instructions) {
            for (int i = 0; i < instruction.second; i++) {
                if (instruction.first == 'R') head += {1, 0};
                else if (instruction.first == 'D') head += {0, 1};
                else if (instruction.first == 'L') head += {-1, 0};
                else if (instruction.first == 'U') head += {0, -1};
